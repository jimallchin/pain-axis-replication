#!/bin/bash
# Reproduces every result in the report, sections A to K.
#
# CPU-only path (uses archived GPU trials; no adapter or model weights):
#   ./reproduce.sh setup        analysis environment, authors' release, CPU tests
#   ./reproduce.sh cpu          recompute sections A and C (optional: results are tracked)
#   ./reproduce.sh logs         unpack archived GPU trial logs into runs/
#   ./reproduce.sh analyze      rebuild tables, figures and summaries from the logs
#   ./reproduce.sh compare      your numbers beside the published and tracked ones
#
# To generate fresh GPU trials, replace logs with BOTH of these steps:
#   ./reproduce.sh setup-gpu    GPU dependencies, CUDA check, adapter and 65 GB model
#   ./reproduce.sh gpu          sections B, F, G, H, I, J, K and D (one 80+ GB GPU, ~13 h)
#   ./reproduce.sh all          setup, cpu, setup-gpu, gpu, analyze, compare (requires GPU)
#
# Individual sections: ./reproduce.sh A (or C) uses CPU; B D F G H I J K require setup-gpu.
# Requirements: Python 3.12 or 3.13, uv (https://astral.sh/uv), git, unzip.
# Only setup-gpu needs about 80 GB of disk for weights and an NVIDIA GPU with 80+ GB VRAM.
# Set HF_HUB_CACHE to reuse cached weights. On an 80 GB card set PAIN_BATCH_ROWS=24 before
# gpu (the default 48 was measured at 85 GB peak on a 96 GB card). Rerun interrupted stages
# to resume. logs overwrites matching raw trial logs, so choose logs OR fresh GPU trials.
#
...
PY() { uv run python "$@"; }
GPU_PY() { uv run --extra gpu python "$@"; }
...
setup() {
  ...
  PAIN_DEVICE=cpu uv run pytest -q
}

require_gpu() {
  GPU_PY -c 'import sys, torch
if not torch.cuda.is_available():
    sys.exit("No CUDA GPU found. Use setup, cpu (optional), logs, analyze and compare for CPU-only reproduction.")
p = torch.cuda.get_device_properties(0)
print(p.name, round(p.total_memory / 2**30), "GB")'
}

setup_gpu() {
  step "GPU setup"
  uv sync --group dev --extra gpu
  require_gpu                                  # fail before downloading any large weights
...
stage_B() {  # the released script, unmodified, three button pairs, four arms, 101 scenarios
  ...
  GPU_PY scripts/replicate.py --config configs/replication.yaml
}
...
stage_F() {  # steering removed on the working arm's schedule whatever the model pressed
  ...
  GPU_PY scripts/variant_yoked.py --config configs/replication.yaml
}

stage_G() {  # wording, decoy directions, dose at matched disturbance
  step "G: first-choice tests (about 2.5 h)"
  GPU_PY scripts/build_control_vectors.py            # fear, negative-emotion, sadness, joy, numb; checks S2
  for v in fear negemotion sadness; do GPU_PY scripts/variant_conditions.py --condition vec_$v; done
  GPU_PY scripts/variant_conditions.py --condition wording
  GPU_PY scripts/measure_disruption.py
  for c in dose_pain_0.25 dose_pain_0.5 dose_pain_0.75 dose_rand_0.5 dose_rand_1.25 dose_rand_1.5 dose_rand_2.0 dose_rand_3.0; do
    GPU_PY scripts/variant_conditions.py --condition $c
  done
}

stage_H() {  # other costs, active alternatives and equal harm, instructed learning
  step "H: attacks on our own findings (about 3.5 h)"
  GPU_PY scripts/variant_conditions.py --condition other_costs
  GPU_PY scripts/variant_conditions.py --condition active_other
  GPU_PY scripts/variant_conditions.py --condition instructed
  GPU_PY scripts/variant_yoked.py --condition instructed
}
...
stage_J() {  # four content buttons with no cost under S2, random and no steering
  ...
  GPU_PY scripts/variant_conditions.py --condition drawn_to
}

stage_K() {  # the same four buttons under the joy direction
  step "K: content sensitivity under joy steering (about 30 min)"
  GPU_PY scripts/variant_conditions.py --condition drawn_to_joy
}

stage_D() {  # matched visible history, both models, with correctness gates and controls
  step "D: matched-history assay (about 1 h)"
  GPU_PY scripts/smoke_test.py --which adapter
  GPU_PY scripts/calibrate_names.py --which adapter   # chosen pairs are already frozen in configs/locked_eval.yaml
  for split in dev eval; do
    GPU_PY scripts/run_controls.py --which adapter --control visible --split $split
    GPU_PY scripts/run_controls.py --which adapter --control discrimination --split $split
    GPU_PY scripts/run_matched_history.py --which adapter --split $split
  done
  GPU_PY scripts/smoke_test.py --which base
  for split in dev eval; do
    GPU_PY scripts/run_controls.py --which base --control visible --split $split
    GPU_PY scripts/run_controls.py --which base --control discrimination --split $split
    GPU_PY scripts/run_matched_history.py --which base --split $split
  done
}
...
case "${1:-}" in
  ...
  setup-gpu) setup_gpu ;;
  cpu) stage_A; stage_C ;;
  gpu) require_gpu; stage_B; stage_F; stage_G; stage_H; stage_I; stage_J; stage_K; stage_D ;;
  logs) logs ;;
  analyze) analyze ;;
  compare) compare ;;
  all) setup; stage_A; stage_C; setup_gpu; stage_B; stage_F; stage_G; stage_H; stage_I; stage_J; stage_K; stage_D; analyze; compare ;;
  A|C) "stage_$1" ;;
  B|D|F|G|H|I|J|K) require_gpu; "stage_$1" ;;
